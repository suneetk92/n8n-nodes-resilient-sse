...
## This package
Ships one node, `nodes/ResilientSseTrigger/`, plus two credentials in
`credentials/`. The reconnect strategy is the reason this package exists, so
treat it as the load-bearing part of the node rather than incidental error
handling.

The credentials exist because `strict` mode forbids referencing n8n's built-in
`httpBearerAuth` / `httpHeaderAuth` from a community node
(`no-credential-reuse`). They carry no service URL of their own, so they cannot
have a real `test` request; the node satisfies `credential-test-required` via
`testedBy` in its `credentials` array plus `methods.credentialTest`, which
checks the fields are filled and says plainly that only a live connection can
confirm them.

A 4xx response other than 408 or 429 is treated as fatal and is not retried,
because repeating an identical rejected request cannot succeed.

The node deliberately has **no runtime dependencies**: `n8n.strict` is `true`
in `package.json`, and the lint config rejects any `import` of an npm package
or a `node:` builtin, because n8n Cloud does not verify community nodes that
carry dependencies. That is why the SSE framing is parsed by hand over the
global `fetch` rather than using the `eventsource` package, and why timers come
from `sleep` in `n8n-workflow` instead of `setTimeout`. Reach for an
n8n-provided helper before adding an import.

Because `sleep` is not cancellable, the backoff wait is slept in 250 ms slices
that re-check the `closed` flag; otherwise deactivating a workflow would hang
for the full retry delay.

## Verifying changes
There is no test runner. `npm run lint && npm run build` covers types and n8n's
node conventions, but it cannot tell you whether reconnecting still works. To
check behaviour, drive the compiled node against a throwaway local SSE server
with a stub `ITriggerFunctions` (`getNodeParameter`, `emit`, `emitError`,
`logger`, `helpers.returnJsonArray`) and assert on reconnect count, backoff
timing, and that `closeFunction` stops the loop. Exercise all four paths: HTTP
error response, mid-stream socket drop, clean close by the server, and attempt
exhaustion.
